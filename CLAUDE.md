...
- **The category defaults live in code, not in `action.yml`**: they are `DEFAULT_CATEGORY_GLOBS` (`src/tally.ts`), and `src/__tests__/tally.test.ts` drives its cases off that constant rather than a copy, so a case is a claim about what users get. The `<category>-patterns` and `extra-<category>-patterns` inputs default to `''` and are folded into that constant by `resolveCategoryGlobs`, which is also why an empty input list reads as "unset" rather than "empty this category" -- `getMultilineInput` cannot tell the two apart. Putting the defaults in the manifest instead would re-add the `yaml` devDependency that removal retired (issue #5).
